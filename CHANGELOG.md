...
### 2025-01-06 新增
- **API Key 管理系统**：支持程序化访问（如 PicGo 图床、备份脚本等）
  - 创建、编辑、删除和启用/禁用 API Key
  - 支持 Bearer Token 和 X-API-Key 两种认证方式
  - 路径权限控制：可配置全部目录或指定目录
  - 只读模式：限制密钥只能下载，不能上传/删除
  - 有效期设置：永久/7天/30天/90天/1年
  - 密钥预览和最后使用时间记录
  - 前端管理界面：完整的密钥管理对话框

### 优化
- 认证系统重构：支持 Basic Auth 和 API Key 双重认证
- 管理工具权限检查改为异步方式，支持 API Key 认证
